...
const PendingLeaveHistory = () => {
  ...
  const currentPageData = pendingLeaveApplications.slice(
    startIndex,
    startIndex + pageSize
  );
    ...
    <>
              ...
              {currentPageData.map((item, index) => (
                <tr
                  ...
                  className={`text-center ${
                    item.status === "Pending" ? "dark:bg-darkNavbarBg" : ""
                  }`}
                >
      ...
      {pendingLeaveApplications.length > 0 && (
        <div className="mt-4 flex justify-end">
          <Pagination
            current={currentPage}
            total={pendingLeaveApplications.length}
            pageSize={pageSize}
...
export default PendingLeaveHistory;
