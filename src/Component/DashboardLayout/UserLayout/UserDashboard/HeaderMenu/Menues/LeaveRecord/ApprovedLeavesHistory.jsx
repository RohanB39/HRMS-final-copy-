...
const ApprovedLeavesHistory = () => {
  const dispatch = useDispatch();
  const { empDetails = {}, loading, error } = useSelector((state) => state.employees);
  const leaveApplications = empDetails.leaveApplications || []; // Default to an empty array if undefined
  ...
  const currentPageData = approvedLeaveApplications.slice(
    startIndex,
    startIndex + pageSize
  );
  ...
  return (
    <>
      <div className="overflow-x-auto mt-4">
        {approvedLeaveApplications.length === 0 ? (
          // Show empty state if there are no approved leaves
          <Empty description="No approved leave applications" />
        ) : (
      ...
      <div className="mt-4 flex justify-end">
        <Pagination
          current={currentPage}
          total={approvedLeaveApplications.length}
          pageSize={pageSize}
...
export default ApprovedLeavesHistory;
